...
local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/dothenotive/internal/main/library.lua"))()
getgenv().api = loadstring(game:HttpGet("https://raw.githubusercontent.com/dothenotive/internal/main/api.lua"))()
local bssapi = loadstring(game:HttpGet("https://raw.githubusercontent.com/dothenotive/internal/main/bssapi.lua"))()
local httpreq = (syn and syn.request) or http_request or (http and http.request) or request

if not isfolder("kocmoc") then
	makefolder("kocmoc")
end
if not isfolder("kocmoc/premium") then
	makefolder("kocmoc/premium")
end
if not isfolder("kocmoc/plantercache") then
	makefolder("kocmoc/plantercache")
end
...
function maskequip(mask)
	if rtsg()["EquippedAccessories"]["Hat"] == mask then
		return
	end
...
function equiptool(tool)
	if rtsg()["EquippedCollector"] == tool then
		return
	end
...
repeat
	task.wait(0.1)
until player.PlayerGui.ScreenGui:FindFirstChild("Menus")
	...
	if farmclosest then
		for i = 0, 10 do
			...
			for e, r in next, game.Workspace.Collectibles:GetChildren() do
				if r:FindFirstChild("farmed") then
					continue
				end
				itb = false
				...
				if not itb and findField(r.Position) == findField(api.humanoidrootpart().Position) then
					if closesttoken.Distance then
						if (r.Position - api.humanoidrootpart().Position).magnitude < closesttoken.Distance then
							closesttoken = {
								Token = r,
								Distance = (r.Position - api.humanoidrootpart().Position).magnitude
							}
						end
					else
						closesttoken = {
		...
		for e, r in next, game.Workspace.Collectibles:GetChildren() do
			if r:FindFirstChildOfClass("Decal") then
				...
				if aaaaaaaa ~= nil and api.findvalue(kocmoc.priority, aaaaaaaa) then
					if r.Name == player.Name and not r:FindFirstChild("got it") or tonumber((r.Position - api.humanoidrootpart().Position).magnitude) <= temptable.magnitude / 1.4 and not r:FindFirstChild("got it") then
						farm(r)
...
function getballoons()
	if temptable.doingbubbles or temptable.doingcrosshairs then
		return
	end
...
function getflower()
	...
	if tonumber((flowerrrr - api.humanoidrootpart().Position).magnitude) <= temptable.magnitude / 1.4 and tonumber((flowerrrr - fieldposition).magnitude) <= temptable.magnitude / 1.4 then
		if temptable.running == false then
			if kocmoc.toggles.loopfarmspeed then
				player.Character.Humanoid.WalkSpeed = kocmoc.vars.farmspeed
			end
...
function getfuzzy()
	pcall(function()
		for i, v in next, game.workspace.Particles:GetChildren() do
			if v.Name == "DustBunnyInstance" and temptable.running == false and tonumber((v.Plane.Position - api.humanoidrootpart().Position).magnitude) < temptable.magnitude / 1.4 then
...
function dobubbles()
	...
	if temptable.started.ant or temptable.started.vicious or temptable.converting or temptable.planting then
		return
	end
...
function docrosshairs()
	...
	if temptable.started.ant or temptable.started.vicious or temptable.converting or temptable.planting or temptable.started.monsters then
		return
	end
	local savespeed = kocmoc.vars.walkspeed
	for _, v in pairs(game.Workspace.Particles:GetChildren()) do
							...
							if image.ImageTransparency == 0 then
								if kocmoc.toggles.tptonpc then
									api.humanoidrootpart().CFrame = CFrame.new(
                                        v.Platform.Position.X, v.Platform.Position.Y + 3, v.Platform.Position.Z)
									task.wait(1)
								else
...
autofarmtoggle:CreateKeybind("U", function(Key)
end)
guiElements["toggles"]["autodig"] = farmo:CreateToggle("Autodig", nil, function(State)
	kocmoc.toggles.autodig = State
end)
guiElements["vars"]["autodigmode"] = farmo:CreateDropdown("Autodig Mode", {
	"Normal",
	"Collector Steal"
}, function(Option)
	kocmoc.vars.autodigmode = Option
end)
...
guiElements["vars"]["autouseMode"] = contt:CreateDropdown("Bag Reduction Mode", {
	...
	"Just Snowflakes",
	"Just Coconuts",
	"Snowflakes and Coconuts",
	"Tickets and Snowflakes",
	"Tickets and Coconuts",
	"All"
}, function(Select)
	kocmoc.vars.autouseMode = Select
end)
...
guiElements["toggles"]["autosprinkler"] = farmo:CreateToggle("Auto Sprinkler", nil, function(State)
	kocmoc.toggles.autosprinkler = State
end)
guiElements["toggles"]["farmbubbles"] = farmo:CreateToggle("Farm Bubbles", nil, function(State)
	kocmoc.toggles.farmbubbles = State
end)
guiElements["toggles"]["farmflame"] = farmo:CreateToggle("Farm Flames", nil, function(State)
	kocmoc.toggles.farmflame = State
end)
guiElements["toggles"]["farmcoco"] = farmo:CreateToggle("Farm Coconuts & Shower", nil, function(State)
	kocmoc.toggles.farmcoco = State
end)
guiElements["toggles"]["collectcrosshairs"] = farmo:CreateToggle("Farm Precise Crosshairs", nil, function(State)
	kocmoc.toggles.collectcrosshairs = State
end)
guiElements["toggles"]["fastcrosshairs"] = farmo:CreateToggle("Smart Precise Crosshairs [" .. Danger .. "]", nil, function(State)
	kocmoc.toggles.fastcrosshairs = State
end)
guiElements["toggles"]["farmfuzzy"] = farmo:CreateToggle("Farm Fuzzy Bombs", nil, function(State)
	kocmoc.toggles.farmfuzzy = State
end)
guiElements["toggles"]["farmunderballoons"] = farmo:CreateToggle("Farm Under Balloons", nil, function(State)
	kocmoc.toggles.farmunderballoons = State
end)
guiElements["toggles"]["farmclouds"] = farmo:CreateToggle("Farm Under Clouds", nil, function(State)
	kocmoc.toggles.farmclouds = State
end)
farmo:CreateLabel("")
guiElements["toggles"]["honeymaskconv"] = farmo:CreateToggle("Auto Honey Mask", nil, function(bool)
	kocmoc.toggles.honeymaskconv = bool
end)
guiElements["vars"]["defmask"] = farmo:CreateDropdown("Default Mask", MasksTable, function(val)
	kocmoc.vars.defmask = val
end)
guiElements["vars"]["deftool"] = farmo:CreateDropdown("Default Tool", collectorstable, function(val)
	kocmoc.vars.deftool = val
end)
guiElements["toggles"]["autoequipmask"] = farmo:CreateToggle("Equip Mask Based on Field", nil, function(bool)
	kocmoc.toggles.autoequipmask = bool
end)
...
guiElements["toggles"]["autodispense"] = farmt:CreateToggle("Auto Dispenser [⚙]", nil, function(State)
	kocmoc.toggles.autodispense = State
end)
guiElements["toggles"]["autoboosters"] = farmt:CreateToggle("Auto Field Boosters [⚙]", nil, function(State)
	kocmoc.toggles.autoboosters = State
end)
guiElements["toggles"]["clock"] = farmt:CreateToggle("Auto Wealth Clock", nil, function(State)
	kocmoc.toggles.clock = State
end)
...
guiElements["toggles"]["freeantpass"] = farmt:CreateToggle("Auto Free Antpasses", nil, function(State)
	kocmoc.toggles.freeantpass = State
end)
guiElements["toggles"]["farmsprouts"] = farmt:CreateToggle("Farm Sprouts", nil, function(State)
	kocmoc.toggles.farmsprouts = State
end)
guiElements["toggles"]["farmpuffshrooms"] = farmt:CreateToggle("Farm Puffshrooms", nil, function(State)
	kocmoc.toggles.farmpuffshrooms = State
end)
farmt:CreateToggle("Farm Snowflakes ["..Danger.."]", nil, function(State) kocmoc.toggles.farmsnowflakes = State end)
guiElements["toggles"]["farmrares"] = farmt:CreateToggle("Teleport To Rares [" .. Danger .. "]", nil, function(State)
	kocmoc.toggles.farmrares = State
end)
guiElements["toggles"]["autoquest"] = farmt:CreateToggle("Auto Accept/Confirm Quests [⚙]", nil, function(State)
	kocmoc.toggles.autoquest = State
end)
guiElements["toggles"]["autodoquest"] = farmt:CreateToggle("Auto Do Quests [⚙]", nil, function(State)
	kocmoc.toggles.autodoquest = State
end)
guiElements["toggles"]["honeystorm"] = farmt:CreateToggle("Auto Honeystorm", nil, function(State)
	kocmoc.toggles.honeystorm = State
end)
farmt:CreateLabel(" ")
...
guiElements["toggles"]["autoplanters"] = plantersection:CreateToggle("Auto Planters", nil, function(State)
	kocmoc.toggles.autoplanters = State
end)
guiElements["toggles"]["blacklistinvigorating"] = plantersection:CreateToggle("Blacklist Invigorating", nil, function(State)
	kocmoc.toggles.blacklistinvigorating = State
end)
guiElements["toggles"]["blacklistcomforting"] = plantersection:CreateToggle("Blacklist Comforting", nil, function(State)
	kocmoc.toggles.blacklistcomforting = State
end)
guiElements["toggles"]["blacklistmotivating"] = plantersection:CreateToggle("Blacklist Motivating", nil, function(State)
	kocmoc.toggles.blacklistmotivating = State
end)
guiElements["toggles"]["blacklistrefreshing"] = plantersection:CreateToggle("Blacklist Refreshing", nil, function(State)
	kocmoc.toggles.blacklistrefreshing = State
end)
guiElements["toggles"]["blacklistsatisfying"] = plantersection:CreateToggle("Blacklist Satisfying", nil, function(State)
	kocmoc.toggles.blacklistsatisfying = State
end)
...
guiElements["toggles"]["plasticplanter"] = plantersection:CreateToggle("Blacklist Plastic Planter", nil, function(State)
	kocmoc.toggles.plasticplanter = State
end)
guiElements["toggles"]["candyplanter"] = plantersection:CreateToggle("Blacklist Candy Planter", nil, function(State)
	kocmoc.toggles.candyplanter = State
end)
guiElements["toggles"]["redclayplanter"] = plantersection:CreateToggle("Blacklist Red Clay Planter", nil, function(State)
	kocmoc.toggles.redclayplanter = State
end)
guiElements["toggles"]["blueclayplanter"] = plantersection:CreateToggle("Blacklist Blue Clay Planter", nil, function(State)
	kocmoc.toggles.blueclayplanter = State
end)
guiElements["toggles"]["tackyplanter"] = plantersection:CreateToggle("Blacklist Tacky Planter", nil, function(State)
	kocmoc.toggles.tackyplanter = State
end)
guiElements["toggles"]["pesticideplanter"] = plantersection:CreateToggle("Blacklist Pesticide Planter", nil, function(State)
	kocmoc.toggles.pesticideplanter = State
end)
guiElements["toggles"]["petalplanter"] = plantersection:CreateToggle("Blacklist Petal Planter", nil, function(State)
	kocmoc.toggles.petalplanter = State
end)
...
guiElements["toggles"]["docustomplanters"] = customplanterssection:CreateToggle("Custom Planters", nil, function(State)
	kocmoc.toggles.docustomplanters = State
end)
...
mobkill:CreateToggle("Train Snail", nil, function(State)
	...
	if State then
		api.humanoidrootpart().CFrame = CFrame.new(
            fd.Position.X, fd.Position.Y - 20, fd.Position.Z)
	else
		api.humanoidrootpart().CFrame = CFrame.new(
            fd.Position.X, fd.Position.Y + 2, fd.Position.Z)
	end
end)
guiElements["toggles"]["killmondo"] = mobkill:CreateToggle("Kill Mondo", nil, function(State)
	kocmoc.toggles.killmondo = State
end)
guiElements["toggles"]["killvicious"] = mobkill:CreateToggle("Kill Vicious", nil, function(State)
	kocmoc.toggles.killvicious = State
end)
guiElements["toggles"]["killwindy"] = mobkill:CreateToggle("Kill Windy", nil, function(State)
	kocmoc.toggles.killwindy = State
end)
local autokillmobstoggle = mobkill:CreateToggle("Auto Kill Mobs", nil, function(State)
	kocmoc.toggles.autokillmobs = State
end)
autokillmobstoggle:AddToolTip("Kills mobs after x pollen converting")
guiElements["toggles"]["autokillmobs"] = autokillmobstoggle
guiElements["toggles"]["avoidmobs"] = mobkill:CreateToggle("Avoid Mobs", nil, function(State)
	kocmoc.toggles.avoidmobs = State
end)
local autoanttoggle = mobkill:CreateToggle("Auto Ant", nil, function(State)
	kocmoc.toggles.autoant = State
end)
autoanttoggle:AddToolTip("You will need a spark staff; goes to Ant Challenge after pollen converting")
...
serverhopkill:CreateButton("Vicious Bee Serverhopper [" .. Danger .. "][" .. ExploitSpecific .. "]", function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/dothenotive/internal/main/viciousbeehop.lua"))()
...
wayp:CreateDropdown("Monster Teleports", spawnerstable, function(Option)
	...
	api.humanoidrootpart().CFrame = CFrame.new(
        d.Position.X, d.Position.Y + 3, d.Position.Z)
end)
...
wstoggle:CreateKeybind("K", function(Key)
end)
...
jptoggle:CreateKeybind("L", function(Key)
end)
...
extras:CreateButton(
    "Hide nickname",
    function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/not-weuz/Lua/main/nicknamespoofer.lua"))()
    end
)
extras:CreateButton(
    "Boost FPS",
    function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/not-weuz/Lua/main/fpsboost.lua"))()
    end
)
extras:CreateButton(
    "Destroy Decals",
    function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/not-weuz/Lua/main/destroydecals.lua"))()
    end
)
extras:CreateTextBox(
    "Glider Speed",
    "",
    true,
    function(Value)
...
if string.find(string.upper(identifyexecutor()), "SYN") or string.find(string.upper(identifyexecutor()), "SCRIP") then
	...
	visu:CreateButton("Spawn Flame [" .. ExploitSpecific .. "]", function()
		syn.secure_call(function()
			require(game.ReplicatedStorage.LocalFX.LocalFlames).AddFlame(
                player.Character.Humanoid.RootPart.CFrame.p, 10, 1, player.UserId, false)
		end, player.PlayerScripts.ClientInit)
	end)
	visu:CreateButton("Spawn Dark Flame [" .. ExploitSpecific .. "]", function()
		syn.secure_call(function()
			require(game.ReplicatedStorage.LocalFX.LocalFlames).AddFlame(
                player.Character.Humanoid.RootPart.CFrame.p, 10, 1, player.UserId, true)
		end, player.PlayerScripts.ClientInit)
	end)
	...
	visu:CreateLabel("")
	local destroym = true
	visu:CreateToggle("Destroy Map", true, function(State)
		destroym = State
	end)
...
raresettings:CreateButton("Add Token To Rares List", function()
	...
	raresettings:CreateDropdown("Rares List", kocmoc.rares, function(Option)
	end)
end)
...
raresettings:CreateButton("Add Token To Blacklist", function()
	...
	game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Tokens Blacklist Dropdown", true):Destroy()
	raresettings:CreateDropdown("Tokens Blacklist", kocmoc.bltokens, function(Option)
	end)
end)
...
local UIColorPicker = guisettings:CreateColorpicker("UI Color", function(Color)
					...
					elseif lastCharacters == " Slider" then
						task.spawn(function()
							local Tween = game:GetService("TweenService"):Create(
                                obj.Slider.Bar, TweenInfo.new(1), {
...
fieldsettings:CreateButton("Add Field To Blacklist", function()
	...
	fieldsettings:CreateDropdown("Blacklisted Fields", kocmoc.blacklistedfields, function(Option)
	end)
end)
...
guiElements["toggles"]["allquests"] = aqs:CreateToggle("Non-Repeatable Quests", nil, function(State)
	kocmoc.toggles.allquests = State
end)
guiElements["toggles"]["buckobeequests"] = aqs:CreateToggle("Bucko Bee Quests", nil, function(State)
	kocmoc.toggles.buckobeequests = State
end)
guiElements["toggles"]["rileybeequests"] = aqs:CreateToggle("Riley Bee Quests", nil, function(State)
	kocmoc.toggles.rileybeequests = State
end)
guiElements["toggles"]["blackbearquests"] = aqs:CreateToggle("Black Bear Quests", nil, function(State)
	kocmoc.toggles.blackbearquests = State
end)
guiElements["toggles"]["brownbearquests"] = aqs:CreateToggle("Brown Bear Quests", nil, function(State)
	kocmoc.toggles.brownbearquests = State
end)
guiElements["toggles"]["polarbearquests"] = aqs:CreateToggle("Polar Bear Quests", nil, function(State)
	kocmoc.toggles.polarbearquests = State
end)
...
guiElements["toggles"]["tptonpc"] = aqs:CreateToggle("Teleport To NPC", nil, function(State)
	kocmoc.toggles.tptonpc = State
end)
guiElements["toggles"]["autoquesthoneybee"] = aqs:CreateToggle("Include Honey Bee Quests", nil, function(State)
	kocmoc.toggles.autoquesthoneybee = State
end)
guiElements["toggles"]["buyantpass"] = aqs:CreateToggle("Buy Ant Pass When Needed", nil, function(State)
	kocmoc.toggles.buyantpass = State
end)
guiElements["toggles"]["smartmobkill"] = aqs:CreateToggle("Modify Mob Kill To Match Quests", nil, function(State)
	kocmoc.toggles.smartmobkill = State
end)
guiElements["toggles"]["usegumdropsforquest"] = aqs:CreateToggle("Use Gumdrops For Goo Quests", nil, function(State)
	kocmoc.toggles.usegumdropsforquest = State
end)
...
pts:CreateButton("Add Token To Priority List", function()
	...
	pts:CreateDropdown("Priority List", kocmoc.priority, function(Option)
	end)
end)
	...
	if (v:IsA("Part") or v:IsA("MeshPart")) and not temptable.started.ant and not temptable.started.vicious and kocmoc.toggles.autofarm and not temptable.converting and not temptable.planting then
		if v.Name == "WarningDisk" and kocmoc.toggles.farmcoco then
			...
			if v.BrickColor == BrickColor.new("Lime green") then
				task.wait(1.25)
				if (v.Position - api.humanoidrootpart().Position).magnitude > 100 then
					return
				end
	...
	while task.wait() do
		...
		if player.Character:FindFirstChild("ProgressLabel", true) then
			...
			if kocmoc.toggles.autofarm then
				...
				fieldpos = CFrame.new(
                    fieldselected.Position.X, fieldselected.Position.Y + 3, fieldselected.Position.Z)
							...
							while kocmoc.toggles.killmondo and game.Workspace.Monsters:FindFirstChild("Mondo Chick (Lvl 8)") and not temptable.started.vicious and not temptable.started.monsters do
								...
								while game.Workspace.Monsters:FindFirstChild("Mondo Chick (Lvl 8)") do
									...
									api.tween(1, CFrame.new(
                                        mondopition.x, mondopition.y - 60, mondopition.z))
									task.wait(1)
...
game:GetService("RunService").Heartbeat:connect(function()
	if temptable.float and player.Character:FindFirstChild("Humanoid") then
		...
		floatpad.CFrame = CFrame.new(
            api.humanoidrootpart().Position.X, api.humanoidrootpart().Position.Y - 3.75, api.humanoidrootpart().Position.Z)
		task.wait(0)
	else
		floatpad.CanCollide = false
	end
	...
	while task.wait(5) do
		if kocmoc.toggles.docustomplanters then
			local plantercycles = {
				{
					{
						Planter = kocmoc.vars.customplanter11,
						Field = kocmoc.vars.customplanterfield11,
						Percent = kocmoc.vars.customplanterdelay11
					},
					{
						Planter = kocmoc.vars.customplanter12,
						Field = kocmoc.vars.customplanterfield12,
						Percent = kocmoc.vars.customplanterdelay12
					},
					{
						Planter = kocmoc.vars.customplanter13,
						Field = kocmoc.vars.customplanterfield13,
						Percent = kocmoc.vars.customplanterdelay13
					},
					{
						Planter = kocmoc.vars.customplanter14,
						Field = kocmoc.vars.customplanterfield14,
						Percent = kocmoc.vars.customplanterdelay14
					},
					{
						Planter = kocmoc.vars.customplanter15,
						Field = kocmoc.vars.customplanterfield15,
						Percent = kocmoc.vars.customplanterdelay15
					}
				},
				{
					{
						Planter = kocmoc.vars.customplanter21,
						Field = kocmoc.vars.customplanterfield21,
						Percent = kocmoc.vars.customplanterdelay21
					},
					{
						Planter = kocmoc.vars.customplanter22,
						Field = kocmoc.vars.customplanterfield22,
						Percent = kocmoc.vars.customplanterdelay22
					},
					{
						Planter = kocmoc.vars.customplanter23,
						Field = kocmoc.vars.customplanterfield23,
						Percent = kocmoc.vars.customplanterdelay23
					},
					{
						Planter = kocmoc.vars.customplanter24,
						Field = kocmoc.vars.customplanterfield24,
						Percent = kocmoc.vars.customplanterdelay24
					},
					{
						Planter = kocmoc.vars.customplanter25,
						Field = kocmoc.vars.customplanterfield25,
						Percent = kocmoc.vars.customplanterdelay25
					}
				},
				{
					{
						Planter = kocmoc.vars.customplanter31,
						Field = kocmoc.vars.customplanterfield31,
						Percent = kocmoc.vars.customplanterdelay31
					},
					{
						Planter = kocmoc.vars.customplanter32,
						Field = kocmoc.vars.customplanterfield32,
						Percent = kocmoc.vars.customplanterdelay32
					},
					{
						Planter = kocmoc.vars.customplanter33,
						Field = kocmoc.vars.customplanterfield33,
						Percent = kocmoc.vars.customplanterdelay33
					},
					{
						Planter = kocmoc.vars.customplanter34,
						Field = kocmoc.vars.customplanterfield34,
						Percent = kocmoc.vars.customplanterdelay34
					},
					{
						Planter = kocmoc.vars.customplanter35,
						Field = kocmoc.vars.customplanterfield35,
						Percent = kocmoc.vars.customplanterdelay35
					}
				}
			}
			local steps = {
...
task.spawn(function()
	pcall(function()
		...
		local mob2 = panel:CreateButton("Mondo Chick: 00:00", function()
			api.tween(1, game.Workspace.FlowerZones["Mountain Top Field"]
                          .CFrame)
		end)
		...
		while task.wait(1) do
			if kocmoc.toggles.enablestatuspanel then
				...
				local cooldown = require(game.ReplicatedStorage.TimeString)(3600 - (require(game.ReplicatedStorage.OsTime)() - (require(
